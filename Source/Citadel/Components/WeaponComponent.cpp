...
void UWeaponComponent::SetupWeapon()
{
    ...
    {
        UE_LOG(LogTemp, Error, TEXT("%s: Default Weapons has not set!"),
            *Player->GetName());
        return;
    }
    ...
    if (!ActiveWeapon)
    {
        UE_LOG(LogTemp, Error, TEXT("%s: Can't get active weapon!"),
            *Player->GetName());
        return;
    }
...
void UWeaponComponent::Shoot()
{
    ...
    if (AWeaponRocketLauncher* WeaponCasted =
            Cast<AWeaponRocketLauncher>(ActiveWeapon))
        WeaponCasted->Shoot();
}
